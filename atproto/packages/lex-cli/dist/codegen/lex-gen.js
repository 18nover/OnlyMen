...
export function genCommonImports(file, baseNsid) {
...
export function genImports(file, imports, baseNsid) {
...
export function genUserType(file, imports, lexicons, lexUri) {
    ...
    switch (def.type) {
        ...
        case 'token':
            ...
            break;
        case 'object': {
            const ifaceName = toTitleCase(getHash(lexUri));
...
export function genToken(file, lexUri, def) {
    //= /** <comment> */
...
export function genArray(file, imports, lexUri, def) {
...
export function genPrimitiveOrBlob(file, lexUri, def) {
    genComment(file.addTypeAlias({
        name: toTitleCase(getHash(lexUri)),
...
export function genXrpcParams(file, lexicons, lexUri, defaultsArePresent = true) {
    const def = lexicons.getDefOrThrow(lexUri, [
        'query',
        'subscription',
        'procedure',
...
export function genXrpcInput(file, imports, lexicons, lexUri, defaultsArePresent = true) {
...
export function genXrpcOutput(file, imports, lexicons, lexUri, defaultsArePresent = true) {
    const def = lexicons.getDefOrThrow(lexUri, [
        'query',
        'subscription',
        'procedure',
...
export function genRecord(file, imports, lexicons, lexUri) {
    const hash = getHash(lexUri);
    const ifaceName = toTitleCase(hash);
...
export function stripHash(uri) {
    return uri.split('#')[0] || '';
}
export function getHash(uri) {
    return uri.split('#').pop() || '';
}
export function ipldToType(def) {
    if (def.type === 'bytes') {
        return 'Uint8Array';
    }
    return 'CID';
...
export function primitiveOrBlobToType(def) {
    switch (def.type) {
        case 'blob':
            return 'BlobRef';
        case 'bytes':
...
export function primitiveToType(def) {
    switch (def.type) {
        case 'string':
